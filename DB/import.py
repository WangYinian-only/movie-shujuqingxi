    ...
    for sheet in sheets:
        ...
        for i in range(1, row_num):  # 第一行是标题名，对应表中的字段名所以应该从第二行开始，计算机以0开始计数，所以值是1
            ...
            if num >= 10000:  # 每一万条数据执行一次插入
                ...
                sql = "INSERT INTO " + table_name + "(budget,genres,homepage,id,keywords,original_language," \
                                                    "original_title,overview,popularity,production_companies," \
                                                    "production_countries,release_date,revenue,runtime," \
                                                    "spoken_languages," \
                                                    "status,tagline,title,vote_average,vote_count) VALUES(%s,%s,%s," \
                                                    "%s,%s,%s," \
        ...
        sql = "INSERT INTO " + table_name + "(budget,genres,homepage,id,keywords,original_language," \
                                            ...
                                            "status,tagline,title,vote_average,vote_count) VALUES(%s,%s,%s,%s,%s,%s," \
                                            "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) "
        cursor.executemany(sql, list)  # 执行sql语句
...
if __name__ == '__main__':
    store_to('import_movies', 'movies', 'tmdb_5000_movies.xlsx')
